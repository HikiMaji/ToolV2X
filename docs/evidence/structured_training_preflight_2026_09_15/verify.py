...
assert all(checks[k] for k in ('rng','progress','data_binding','input_layout','config','driver_spec')),checks
parameter_differences={k:float((a['model_state'][k]-b['model_state'][k]).abs().max()) for k in a['model_state']}
restore_events=[e for e in lines('resumed_trace.jsonl') if e['kind'].startswith('restore_')]
assert {e['kind'] for e in restore_events}=={'restore_model','restore_optimizer','restore_rng'}
assert all(e['exact'] for e in restore_events)
...
assert T._same(initial['model_state'],T._load(BASE/'interrupted/checkpoint_000000.pt')['model_state'])
...
print('All checkpoints load; exact restore verified; independent CUDA lineages compared without tolerance.',flush=True)
...
pf_exact=measurements[0]==measurements[1]
pf_max_difference=max(abs(x-y) for da,db in zip(measurements[0]['measurements'],measurements[1]['measurements']) for pa,pb in zip(da['waypoints'],db['waypoints']) for x,y in zip(pa,pb))
results={m:read(m+'_result.json') for m in traces}
summary=dict(version='toolv2x_structured_training_preflight_result_v1',status='bounded_preflight_completed_with_reproducibility_limit',
    checkpoint_restoration_exact=True,restore_events=restore_events,
    cross_lineage_parameter_max_abs_difference=max(parameter_differences.values()),
    cross_lineage_different_parameter_tensors=sum(v!=0 for v in parameter_differences.values()),
    pre_interruption_comparison=read('step2_comparison.json'),
    full_scale_ready_as_configured=False,
    ...
    source_files_byte_identical=source_count,archive_tasks_content_equal=len(sources),feature_files_equal=len({r['inputs']['feature_path'] for r in rows}),
    reloaded_pf_measurements_exact=pf_exact,reloaded_pf_max_waypoint_difference_m=pf_max_difference,validation_history=a['report']['validation_history'],
    ...
    limitations=['Four optimization steps are training mechanics evidence, not method quality.',
      ...
      'Checkpoint loading is exact; independent CUDA lineages are not bitwise equal even before interruption; no exact-continuation claim.',
