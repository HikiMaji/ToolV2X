...
for arm in ('Ego','P','F','PF'):
    ...
    norms={'history':0.,'forecast':0.,'anchor_only_state':0.};locations=dict.fromkeys(norms,0)
        ...
        for group in prepared['admission_report']['field_groups']:
            ...
            for location in group['tensor_locations']:
                ...
                if name=='observations':
                    kind='history' if any(ref['field_kind']=='history' for ref in group['primary_refs']) else 'anchor_only_state'
                    s=location['source_slot'];mask=batch['observation_mask'][0,e,s]
                else:
