"""Independent verification of the bounded real one-epoch fit."""
...
assert len(steps)==64 and [r['optimizer_steps'] for r in steps]==list(range(1,65))
assert len(train)==128 and all(r['role']=='train' and math.isfinite(r['loss']) for r in train)
...
initial=T._load(BASE/'training/checkpoint_000000.pt');final=T._load(BASE/'training/checkpoint_000064.pt')
assert final['progress']['optimizer_steps']==64 and final['progress']['epoch']==1
assert final['config']==config and config['save_interval']==config['validation']['interval_batches']==500
assert [x['batches'] for x in final['report']['validation_history']]==[0,64]
...
planner=T.load_structured_planner(BASE/'training/checkpoint_000064.pt',device='cuda')
...
result=dict(version='toolv2x_structured_driver_small_fit_result_v1',status='completed_one_epoch',
    optimizer_steps=64,epochs_completed=1,train_rows_consumed=128,validation_rows=32,
