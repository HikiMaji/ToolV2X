...
2026-09-10 后续语义补充：当前窗口 `valid` 和 P 报文 `history_valid` 表示该时刻存在跟踪输出，包含 AB3DMOT 在漏检帧维持的因果预测状态；它们不是检测匹配掩码。现有缓存未额外记录该匹配位。若训练需要区分实测/跟踪预测，应先扩展跟踪导出并生成新版本缓存。P/F 同信息重算仅适用于双方可见完整提供方历史的条件；P 按 ROI 限制历史时，F 仍使用提供方完整上下文。详见 [本机复核](review_9_10_verification.md) 与 [修复说明](review_9_10_fixes.md)。下文保留早期协议审查时点的结果和待办。
