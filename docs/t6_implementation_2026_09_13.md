...
> 后续修复（2026-09-13）：[9-13-2 定向修复记录](t6_review_fixes_2026_09_13.md) 更新了单轮预算化摘要、bundle v2 聚合响应上限、策略前可支付动作 mask，以及排除保存的分段控制计时。新有控制评价为 measured_stages_v3；旧无计时版本的数据仅保留上报时间诊断，不当作可信计算成本。237 项轻量回归通过。本文保留原 T6 批次的实现和验证历史，当前相关语义以修复记录为准。
