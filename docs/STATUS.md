...
**正在执行：MTR 多种子稳定性对照。** 用户要求开始下一步后，已按 [固定协议](mtr_stability_plan.md) 启动 full/alternating × seed 20/21/22 六组原网络训练。每次 10 轮、每轮 996 次有效更新；采样重复和监督中心数单列。启动时入口检查为完整 85 项、轻量 63 项通过。运行及逐轮进度在 `outputs/mtr_stability_v1/`；六组未全部结束前，不把某个最佳轮作为已冻结结论。

**并行驾驶入口：** 已执行 GoT/V2V-LLM × direct/Q8→Q9 的真实原 7B 单帧接入，16 次动作尝试及失败均保留，见 [诊断](driving_decoder_results.md)。已补一次共享参数更新检查；较长 P 样本在当前 SDPA 训练回退路径 OOM，尚无 optimizer 更新，正在准备原训练脚本使用的 FlashAttention 2。常规完整 91 项、轻量 66 项通过不代替真实 backward/更新完成。正式共享驾驶适配、同预算 2×2 选型与多帧四动作质量比较仍未完成。
...
完整/ROI 按轮次交替属于首轮适配设置，不等于 P/F 或查询机制收益。训练曲线明显波动；首轮 ROI 同目标下完整上下文没有更低的平均预测误差。当前按顶部六组协议检查稳定性，再决定冻结权重、物化工具证据并适配共享驾驶模型。查询策略和闭环继续后置。所有新输出保存在新目录；此次 GitHub 内容与完成边界见 [更新说明](github_update_2026_09_11.md)。
