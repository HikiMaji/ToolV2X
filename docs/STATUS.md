...
**本次 GitHub 发布核验：** 完整框架代码、文档和精选真实文本已整理，见 [更新范围](github_update_framework_2026_09_11.md)。新增两项真实归档检查后，本机完整 117 项与轻量 89 项通过；从 Git 暂存树导出的无模型、无外部数据副本也通过 89 项。后台全量任务仍独立运行，GitHub 仅记录 [发布时进度](../outputs/framework_baseline_v1/progress_at_github_update.json)。
...
**当前运行：全量单配置基础适配流程已启动。** 2026-09-11 19:03（UTC+8）启动 `outputs/framework_baseline_v1/`，执行该目录下保存的代码。首先为清理后的 3095 帧执行五策略，然后自动导出 14935 条训练行与 540 条验证输入、从原 GoT 初始化训练最多三轮、选取验证损失检查点、对 108 个开发验证帧进行五策略生成及评价。实际阶段读取 `status.json`，准备进度读取 `episodes/progress.json`；不要重复启动或把计划后续步骤写为完成。主进程初始 PID 8392，使用前应重新核对。下一次继续这套配置，不启动 2×2 选型矩阵、RSU/I 或新的学习策略训练。
