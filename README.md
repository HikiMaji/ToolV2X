...
**最新实现（09-13）：** [T1/T2 已完成](docs/t1_t2_implementation_2026_09_13.md)：版本化 ExecutionSpec、current/change 任务检索、独立 v2 P/F 服务、真实远端回执去重和完整 UTF-8 字节预算。新增 36 项契约测试，轻量回归 126/126 通过；旧 v1 保留。T3 的 E/derived/Z ledger、GoT 交替修订与请求价值模块尚未实施；没有新训练或模型实验。审查以 [两份设计与实施边界](docs/github_review.md) 为入口。
