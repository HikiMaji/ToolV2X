...
**最新研究（09-15，导航接入）：** 已核对参考源码、现有 3,095 条准备记录及 3 条原生导航样本；建议局部路线目标点通过普通 MLP token 接入共同驾驶器。导航尚未实现或训练，先完成来源/坐标绑定；小样本训练退化排查后置。见[导航方案与边界](docs/navigation_research_2026_09_15.md)。本次发布包含验收修复、训练预检/结果和导航研究的代码或文档及精简证据；下方未推送等表述保留各历史阶段状态。

**最新训练结果（09-15，数值 v2 20 轮）：** 原 16 训练帧/4 开发帧已完成 1280 次更新并独立核验，已停止。开发 got_prefix_L2_avg 从初始 0.397169 米变为最终 0.528012 米，选模仍为第 0 步；无全量扩帧或新远端采集，不支持方法收益结论。见[训练结果与证据](docs/structured_driver_20ep_fit_2026_09_15.md)。

**最新训练（09-15，数值v2一轮小批适配）：** 现有20帧四条件归档完成1轮/64次更新，128条训练实际覆盖Ego/P/F/PF，32条既有开发阶段行单独验证；0/64步完整权重保留，选模为第0步。模型和原任务独立核验完成，已停止，不自动追加轮次或全量采集。此为小样本固定证据适配，不是ToolV2X方法收益验证。见[结果与证据](docs/structured_driver_small_fit_2026_09_15.md)。未提交/推送。

**最新预检（09-15，数值 v2 真实小批训练）：** 20帧/80任务导出160阶段行，128训练/32开发验证，8/2录制隔离；连续4步与2+2步恢复均完成，两支线合计8次更新；检查点参数/优化器/RNG精确加载，但独立CUDA支线在中断前已有微小参数差，不宣称逐位续跑对照通过。真实P历史/F预测梯度非零，所选阶段32/32合法，但4步开发误差未改善，选模仍为0步。归档与周期检查点的正式规模超出现有存储，先调整存储表示和训练日程；未全量采集/长训、未提交推送。见[预检结果与后续边界](docs/structured_training_preflight_2026_09_15.md)。

**最新修正（09-15，排除原因审计）：** 新派生字段明确区分 ego 过滤、实体容量和预测集合容量，并分别记录间接依赖与完全排除。80 个旧任务、160 个阶段复核确认原有审计字段和任务文件不变；补充 P→F / F→P 的角色转换及 prior 依赖回归。见[实现与验证](docs/admission_reason_fix_2026_09_15.md)。本批无训练、新生成或重新采集，未提交/推送。

**最新执行（09-15，数值驾驶 v2 修复）：** 同一预声明 20 帧初始轨迹 20/20 合法，Ego/P/F/PF 共 80/80 任务及归档审计通过；实际 160 次数值驾驶、40 P＋40 F 请求，全部 60 个协作任务有远端字段入模。完整轻量 347 项、受影响模型/交互 83 项通过。新模型使用因果运动基准与积分残差，仍为未训练 seed7，真实优化步骤为 0；本批已停止，未启动全量采集/长训练。旧 v1 浮点异常未逐一复现，新 v2 统一角度计算并保留严格校验。见[修复与结果](docs/structured_driver_fix_2026_09_15.md)；本次 GitHub 发布包含修复、测试与验收证据；下方保留历史阶段记录。

**最新执行（09-15，Stage B）：** 20 帧 Ego/P/F/PF 真实验收及最多 3 轮短启动已完成，累计 24 个优化步骤，0/8/16/24 步完整检查点均保留并核验。最终初始轨迹合法率仍为 0/20，实际远端请求为 0，已按上限停止；未启动正式长训练或全量四条件采集。初始间歇性张量一致性错误仍未定位，后续完整审计通过不等于根因修复。见[结果与停止点](docs/structured_driver_acceptance_2026_09_15.md)。本轮未改生产代码，未提交/推送；以下为历史阶段记录，Stage A 的 prepared_not_executed 仅描述当时准备快照。
...
**最新完成（09-14，共同 receiver 轮询与两帧真实联调）：** 新增可选 `toolv2x_receiver_v2`，所有策略共用目标轮询，默认 v1 保留。冻结原 GoT/MTR，在原两帧对旧/新 receiver 各执行 Ego、诊断交替、诊断单轮，共 12/12 任务成功、24 次实际 GoT，完整模型环境 358/358、轻量 302/302 通过。新轮询使最终覆盖从 1 个目标变成 2 个；两帧均改变交替的第二次 F 检索，g5526 的交替/单轮最终证据与轨迹出现差异。误差变化有好有坏，不支持 receiver 或序贯方法收益结论。见 [实现](docs/receiver_round_robin_implementation_2026_09_14.md) 与 [真实结果及轨迹图](docs/receiver_round_robin_smoke_2026_09_14.md)。本批无训练、无扩帧，不自动推送；第一次数据根目录配置失败单独保留。

**最新完成（09-14，9-13-3）：** 已修复 bundle 训练的原始 research role 校验，以及 alternating targets 对真实方案、独立离线标签和冻结 utility 的复算。完整轻量 300/300、CPU 合成契约 10/10 通过。既有 T9 两帧 admission 审计定位了 provider 差异在入模时消失的原因；同预算轮询模拟可多覆盖一个目标，但没有新驾驶结果。详见 [修复与完整审计回复](docs/review_9_13_3_response.md)。本批停止于修复和离线诊断，没有实际训练、采集、模型生成或 receiver 修改，本批尚未推送；下方 T9 准备与真实联调已经发布到 main，旧“未推送”等措辞保留当时状态。
...
ToolV2X 研究车辆如何按驾驶任务需要调用邻车的感知 P 与预测 F。**CMP 原 MotionTransformer**继续提供预测能力；驾驶端保留原 V2V-GoT 路径，并新增可选的结构化数值路径。核心待验证机制是：真实远端返回改变本车方案，再影响下一次请求或 STOP。关联、编码和轨迹网络是各对照共用的驾驶基础。
...
新的数值路径使用显式 `toolv2x_interaction_v2` 和完整 `StructuredDriverSpec`。实现状态与验证结果见 [结构化驾驶记录](docs/structured_driver_implementation_2026_09_15.md)。

```text
本车当前/过去检测特征、运动与合法历史 ── 共享数值驾驶器 → τ0
                                                      ↓
任务参数请求 P / F → 实际返回 → 对齐/关联/编码 → 同一驾驶器(τ0) → τ1
                                                      ↓
                                   根据实际修订请求第二次或 STOP

独立未来自车位姿 → 离线六点监督；不进入在线输入或先前方案
```

P-state 历史与 F 多模态预测分别编码后进入同一个网络，不先压成文字。新路径默认 `observations_only`，P-local 重算是显式选项。模态和不同来源保留，关联歧义及远端独有目标不会直接删除。当前没有地图/路线分支，检测特征网格也不等于地图。
...
P 提供当前跟踪状态及可用的 1 秒历史；`history_valid` 表示有可用跟踪输出，包含漏检时跟踪器维持的因果预测状态，不能当作检测匹配标记。F 在提供方完整上下文上预测，再返回查询范围内目标。无 ROI 的完整 P 历史在接收端用同一 MTR 重算，是 F 的同信息对照；仅返回 ROI 内历史的 P 不具备 F 使用的完整上下文。消息保留来源与多模态假设。旧 GoT 路径使用文本证据；新数值路径使用结构化编码。查询价值模块已有代码，但新驾驶底座的策略训练和公平效果对照尚未执行。

旧 GoT 驾驶端的实际输入是原检测器的点云浅层特征及文本证据，未提供真实 RGB。原加载器仍初始化 CLIP；占位图像张量用于激活原点云分支。详细执行边界见接入报告。
