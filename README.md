...
**正在运行：** `outputs/framework_baseline_v1/` 已从保存的代码副本启动全量单配置流程：3095 帧五策略任务 → 14935 条训练行、540 条验证输入 → 最多 3 轮共享驾驶适配 → 全验证生成和评价。GitHub 提供 [发布时进度快照](outputs/framework_baseline_v1/progress_at_github_update.json)，本机实时状态为该目录的 `status.json`；快照不会随后台任务自动更新。训练未结束前不能称正式适配完成。学习调度尚未确定，RSU/I 继续后置。
