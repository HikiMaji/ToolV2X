...
def run_task_episode(local_window, local_prediction, motion, features,
                     ...
                     local_provenance, sample_id=None, branch_id='main', token_counter=None):
    ...
    if (service.scene, service.g) != (scene, g) or service.provider == ledger['local_source'] or service.task_records:
    ...
    while True:
        ...
        try:
            ...
            prepared = build_task_plan_input(driver.tokenizer, motion, ledger, feature_tokens,
                                            limits['receiver_spec'], token_counter=token_counter)
        ...
        if decision['tool'] != 'STOP':
            ...
            request = dict(version=TASK_VERSION, request_id='q%d' % len(episode['requests']),
                ...
                tau_new=copy.deepcopy(current['waypoints']),
                tau_old=copy.deepcopy(previous['waypoints']) if decision['mode'] == 'change' else None,
        ...
        begin = perf_counter()
        try:
            response = service.query_task(request)
        except Exception as exc:
            seconds = perf_counter() - begin
            record = next((r for r in service.task_records if r['request'] == request), None)
