...
class V2V4RealMultiEgoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training=True, logger=None, visualize=False):
        ...
        dataset_cache_file_path = (self.dataset_cfg.DATASET_CACHE_DIR + '/v2v4real_multiego_' +
                ...
                for timestamp_idx, (timestamp_key, timestamp_content) in enumerate(ego_cav_content.items()):
                    ...
                    for cav_id in scenario_content.keys():
                        if not os.path.exists(scenario_content[cav_id][timestamp_key]['preprocessed_gt_data']) or \
                            ...
                            print("preprocessed_gt_data or preprocessed_pred_data not exists")
                            continue
        ...
        for car_id, car_states in cars_data.items():
            ...
            for pre, post in zip(valid_indices[:-1], valid_indices[1:]):
                ...
                if car_states[pre + 1][-1] == 1:  # no need for padding
                    continue
                missing_states = V2V4RealMultiEgoDataset.interpolate_missing_frames(car_states[pre], car_states[post],
                                                                                 missing_frames)
    ...
    def interpolate_missing_frames(prev_state, curr_state, missing_frames):
        ...
        for i in range(1, missing_frames + 1):
            ...
            interpolated_state = V2V4RealMultiEgoDataset.interpolate_states(prev_state, curr_state, weight_prev,
                                                                         weight_curr)
    ...
    def interpolate_states(prev_state, curr_state, weight_prev, weight_curr):
        ...
        x = V2V4RealMultiEgoDataset.interpolate_value(prev_state[0], curr_state[0], weight_prev, weight_curr)
        y = V2V4RealMultiEgoDataset.interpolate_value(prev_state[1], curr_state[1], weight_prev, weight_curr)
        z = prev_state[2]
        l = prev_state[3]
        w = prev_state[4]
        h = prev_state[5]
        heading = V2V4RealMultiEgoDataset.interpolate_value(prev_state[6], curr_state[6], weight_prev, weight_curr)
    ...
    def organize_trajectory_timestamp_into_dict(self, gt_data, pred_data, timestamp_keys,
        ...
        center_objects, track_index_to_predict = V2V4RealMultiEgoDataset.get_interested_agents(
        ...
        (obj_trajs_data, obj_trajs_mask, obj_trajs_pos, obj_trajs_last_pos, obj_trajs_future_state,
         ...
         obj_ids) = V2V4RealMultiEgoDataset.create_agent_data_for_center_objects(
    ...
    def create_agent_data_for_center_objects(
        ...
        obj_trajs_data, obj_trajs_mask, obj_trajs_future_state, obj_trajs_future_mask = V2V4RealMultiEgoDataset.generate_centered_trajs_for_agents(
    ...
    def generate_centered_trajs_for_agents(center_objects, obj_trajs_past, obj_types, center_indices, sdc_index,
        ...
        obj_trajs = V2V4RealMultiEgoDataset.transform_trajs_to_center_coords(
        ...
        obj_trajs_future = V2V4RealMultiEgoDataset.transform_trajs_to_center_coords(
    ...
    def generate_prediction_dicts(batch_pred_dicts, output_path=None):
        ...
        Args:
            batch_pred_dicts:
                ...
                pred_trajs: (num_center_objects, num_modes, num_timestamps, 5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default='MTR/tools/cfgs/v2v4real/v2v4real_multiego.yaml',
    ...
    dataset = V2V4RealMultiEgoDataset(dataset_cfg=dataset_cfg, class_names=['VEHICLE'], training=True)
