...
from mtr.models_v2v4real.utils.transformer import transformer_decoder_layer
from mtr.models_v2v4real.utils.transformer import position_encoding_utils
from mtr.models_v2v4real.utils import common_layers
...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.intention_random_init = self.model_cfg.get("INTENTION_RANDOM_INIT", True)
        ...
        self.intention_points, self.intention_query, self.intention_query_mlps = self.build_motion_query(
            self.d_model, random_init=self.intention_random_init
        )
        ...
        temp_layer = common_layers.build_mlps(c_in=self.d_model * 2, mlp_channels=[self.d_model, self.d_model], ret_before_act=True)
    ...
    def build_dense_future_prediction_layers(self, hidden_dim, num_future_frames):
        ...
        self.dense_future_head = common_layers.build_mlps(
            c_in=hidden_dim * 2,
            mlp_channels=[hidden_dim, hidden_dim, num_future_frames * 5], ret_before_act=True
        )
    ...
    def build_motion_query(self, d_model, random_init=True):
        intention_points = intention_query = intention_query_mlps = None

        if random_init:
            intention_points_dict = {}
            intention_points_dict['TYPE_VEHICLE'] = np.random.rand(64, 2)
            intention_points_dict['TYPE_PEDESTRIAN'] = np.random.rand(64, 2)
            intention_points_dict['TYPE_CYCLIST'] = np.random.rand(64, 2)

            intention_points = {}
            for cur_type in self.object_type:
                cur_intention_points = intention_points_dict[cur_type]
                cur_intention_points = torch.from_numpy(cur_intention_points).float().view(-1, 2).to(self.model_cfg.get('DEVICE', 'cuda'))
    ...
    def build_motion_head(self, in_channels, hidden_size, num_decoder_layers):
        motion_reg_head =  common_layers.build_mlps(
            c_in=in_channels,
            mlp_channels=[hidden_size, hidden_size, self.num_future_frames * 5], ret_before_act=True
        )
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        ...
        if obj_fused_feature_valid.size(0) == 1: # obj_fused_feature_valid with the dimension of N times 1024, a workaround when N=1
            obj_fused_feature_valid = obj_fused_feature_valid.repeat(2, 1)  # Repeat the tensor to increase batch size
            pred_dense_trajs_valid = self.dense_future_head(obj_fused_feature_valid)
            pred_dense_trajs_valid = pred_dense_trajs_valid[0].unsqueeze(0)
        else:
            pred_dense_trajs_valid = self.dense_future_head(obj_fused_feature_valid)

        pred_dense_trajs_valid = pred_dense_trajs_valid.view(pred_dense_trajs_valid.shape[0], self.num_future_frames, 5)
        ...
        ret_pred_dense_future_trajs = obj_feature.new_zeros(num_center_objects, num_objects, self.num_future_frames, 5)
